...
from tools.data.parse_file_list import (parse_directory, parse_diving48_splits,
                                        ...
                                        parse_ucf101_splits, parse_tld3_splits,
                                        parse_tld17k_splits)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            'diving48', 'tld3', 'tld17k'
        ],
        help='dataset to be built file list')
    parser.add_argument(
        '--src_folder', type=str,
        default='/changde/data/tld17k/videos_crop_256_avi',
        help='root directory for the frames or videos')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default='train_val',
        choices=['train_val', 'train', 'val', 'test'],
    ...
    parser.add_argument(
        '--out-root-path',
        type=str,
        default='/caok15/Video-Swin-Transformer/data/',
...
def main():
    ...
    if args.dataset == 'tld17k':
        print('tld17k3c')
        splits = parse_tld17k_splits(args.level)
    elif args.dataset == 'tld3':
        print('tld3')
        splits = parse_tld3_splits(args.level)
    ...
    if len(splits) > 1:
        ...
        print('train')
        ind = 0
        filename = f'{args.dataset}3c_train_list_{args.format}.txt'
        print(filename)
        if args.output_format == 'txt':
            print(len(lists[0][ind]))
            with open(osp.join(out_path, filename), 'w') as f:
                f.writelines(lists[0][ind])

        print('val')
        ind = 1
        filename = f'{args.dataset}3c_val_list_{args.format}.txt'
        print(filename)
        if args.output_format == 'txt':
            print(len(lists[0][ind]))
            with open(osp.join(out_path, filename), 'w') as f:
